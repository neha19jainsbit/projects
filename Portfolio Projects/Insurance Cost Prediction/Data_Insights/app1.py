...
@st.cache_data
def load_data():
    data_path = Path(__file__).resolve().parents[1] / "insurance.csv"
...
if page == "Summary Statistics":
    ...
    col1.metric("Average Premium", f"₹{filtered_df.PremiumPrice.mean():,.0f}")
...
# ======================================================
    ...
    st.subheader("Average Premium by Age Group & Diabetes")
    fig = px.bar(
        filtered_df,
        x="AgeGroup",
        y="PremiumPrice",
...
# ======================================================
    ...
    st.subheader("Allergies & Family Cancer History")
    fig = px.bar(
        filtered_df,
        x="KnownAllergies",
        y="PremiumPrice",
